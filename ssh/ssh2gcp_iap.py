...
def gcloud_configure_firewall(project_id: str, inst: dict):
    networks = get_instance_networks(inst)
    network = networks.pop()

    iap_range = "35.235.240.0/20"
    rule_name = "iap-{}".format(network)

    cmd = ["gcloud", "compute", "firewall-rules", "create",
           "--project", project_id, rule_name, "--network", network,
           "--allow=all", "--source-ranges", iap_range]
...
def process_user_request():
    ...
    # Connect / run command
    gcloud_connect_to_instance(project_id, user, instance, command)
